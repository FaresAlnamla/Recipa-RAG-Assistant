...
import { apiClient, AgentAskResponse, StreamEvent } from "@/lib/api";
...
export default function QAEngine() {
  ...
  const clearHistory = () => {
    ...
    if (sessionId) {
      apiClient.clearSession(sessionId).catch((err) => {
  ...
  const handleSubmit = async (e?: FormEvent, manualQuestion?: string) => {
    ...
    try {
      ...
      for await (const event of apiClient.askAgentStream(
        queryText.trim(),
        sessionId
      )) {
        ...
        switch (event.type) {
          case "token":
            ...
            break;
          case "done":
            // Streaming complete - finalize answer and sources
            if (event.answer) {
            ...
            setHistory((prev) =>
              [
                ...prev,
                { question: queryText.trim(), answer: fullAnswer, sources },
              ].slice(-10)
            );
            break;
          case "error":
      ...
      <div className="max-w-7xl mx-auto px-4 w-full relative z-10">
        ...
        <div className="max-w-5xl mx-auto space-y-10">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex gap-4">
                  <Button
                    ...
                    className="flex-1 h-20 text-xl font-bold rounded-xl bg-gray-900 hover:bg-black text-white"
                  >
                    {isLoading ? (
                      <Loader2 className="animate-spin mr-2" />
                    ) : (
                      <>
                        <span className="mr-2">Generate</span> <ArrowRight />
                      </>
                    )}
                  </Button>
                  {isLoading && (
          ...
          {currentAnswer && (
            <Card
              id="answer-panel"
              className="bg-white shadow-lg rounded-2xl animate-in fade-in slide-in-from-bottom-4 duration-500 border border-gray-200"
            >
                  ...
                  <div className="prose prose-lg prose-slate max-w-none">
                    <ReactMarkdown
                      ...
                      components={{
                        ...
                        thead: ({ node, ...props }) => (
                          <thead
                            className="bg-orange-50 text-orange-900 uppercase font-bold"
                            {...props}
                          />
                        ),
                        th: ({ node, ...props }) => (
